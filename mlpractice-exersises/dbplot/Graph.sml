...
struct
  ...
  fun tick (min, max) =
    let
      ...
      val s = w / e
      val step =
        if s >= 15.0 then 5.0 * e
        else if s >= 2.0 then 2.0 * e
        else e
    in
      {
        min = Real.realFloor (min / step) * step,
        step = step
      }
    end
  ...
  fun plot points =
    let
      ...
      val ps = map (fn {x, y} => CIRCLE (xpos x, ypos y, 0.01)) points
    in
      STROKE (COMBINE ps) :: obj
    end

  fun plotHistogram points =
    let
      val xmax = foldl Real.max Real.negInf (map #x points)
      val xmin = foldl Real.min Real.posInf (map #x points)
      val {min = xmin, step = xstep} = tick (xmin, xmax)
      ...
      val () = Dynamic.pp histogram

      val ymin = 0.0
      val ymax = foldl Real.max Real.negInf histogram
      val {min = ymin, step = ystep} = tick (ymin, ymax)
      ...
      val xpos = fn x => (x - xmin) * xscale + xoffset
      val ypos = fn y => (y - ymin) * yscale + yoffset
    ...
    in
      [STROKE (COMBINE [RECT (0.1, 0.05, 0.85, 0.85),
                       COMBINE xt, COMBINE yt]),
              FILL (COMBINE [COMBINE xl, COMBINE yl]),
              STROKE (COMBINE histogramObjs)]
    end
end
