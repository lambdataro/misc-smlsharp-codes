...
fun intersect (ray as { orig, dir }) =
  let
    val (t, {center, ...}) =
      foldl
        (fn (x, z) => if #1 x < #1 z  then x else z)
...
fun parallel_for ({begin, last, size}: { begin: int, last: int, size: int }) (body: int -> unit) =
  if last - begin + 1 <= size then
    body begin
  else
    let
      val w2 = (last - begin + 1) div 2
      val t = Myth.Thread.create (fn _ => (parallel_for { begin = begin, last = begin + w2 - 1, size = size} body; 0))
    in
      (
        parallel_for { begin = begin + w2, last = last, size = size } body;
        Myth.Thread.join t;
        ()
      )
    end
...
val _ =
  parallel_for { begin = 0, last = width - 1, size = cutOff }
    (fn x =>
      parallel_for { begin = 0, last = height - 1, size = cutOff }
        (fn y =>
          (rayTrace (x, y, cutOff, cutOff))
        )
    )
